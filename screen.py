...
class Screen:
    ...
    def _init_graphics(self):
        ...
        scoreTitle = tkinter.Label(frame, text="Score", height=3, width=10)
        scoreTitle.pack()
        scoreFrame = tkinter.Frame(frame, height=32, bd=23, relief=tkinter.SUNKEN)
        scoreFrame.pack()
        score = tkinter.Label(scoreFrame, height=2, width=20, textvariable=self._score_val, fg="Yellow", bg="green")
    ...
    def ontimer(self, func, milli):
        ...
        this function calld at the end of the main game loop.
    ...
    def _bind_keys(self):
        ...
        self._bind_key("Down", self._handle_back)
        self._bind_key("s", self._handle_s)
    ...
    def is_back_pressed(self):
        """
        :returns: True if the back key was pressed, else False
               """

        res = self._downCliks > 0
        self._downCliks -= 1 if res else 0
        return res
    ...
    def is_s_pressed(self):
        """
        :returns: True if the stop key was pressed, else False
        """
        res = self._stopClicks > 0
        self._stopClicks -= 1 if res else 0
        return res
